...
class LinearRecurrenceSequence(SageObject):
    ...
    def __repr__(self):
        ...
        """
        initial = ", ".join(f"u_{i} = {self.initial_conditions[i]}" for i in range(self.degree))
        definition = " + ".join(f"{self.coefficients[i]}*u_{{n+{self.degree-i-1}}}" for i in range(self.degree))
        return f"Linear recurrence sequence defined by: u_{{n+{self.degree}}} = {definition};\nWith initial conditions: {initial}"
    ...
    def __eq__(self, other):
        ...
        EXAMPLES::
            ...
            False
        """
        return (self.initial_conditions == other.initial_conditions) and (self.coefficients == other.coefficients)
    ...
    def minimal_polynomial(self):
        ...
        EXAMPLES::
            ...
            x^3 + 2*x^2 - x - 2
        """
        values = self.initial_conditions+[self(self.degree+i) for i in range(self.degree)]
        return berlekamp_massey(values)
...
class BinaryRecurrenceSequence(LinearRecurrenceSequence):
